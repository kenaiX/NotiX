...
class BuildXXhdpi : AppCompatActivity() {
    ...
    fun dealWithBitmap(bitmap: Bitmap): Bitmap {
        ...
        val paint = Paint()
        val rect = Rect(5, 5, result.width - 5, result.height - 5)
        val rectF = RectF(rect)
        paint.setAntiAlias(true)
        paint.setColor(color)
        cavans.drawRoundRect(rectF, 2f, 2f, paint)
    ...
    fun saveMyBitmap(bitName: String, mBitmap: Bitmap) {
        val f = File("/sdcard/zzz/$bitName.png")
        try {
            f.createNewFile()
        } catch (e: IOException) {
